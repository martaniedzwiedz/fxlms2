...
static struct lf_ring feedback_ref[4];
...
static double feedback_neutralization(int node_id)
{
	...
	for (ui = 0; ui < plate_params.u ; ui++)
		f = lf_fir(&feedback_ref[ui], models[node_id][ui][REFERENCE_CHANNEL], plate_params.feedback_n);
	return f;
}
...
static void parse_command_line(int argc, char *argv[]){
	...
	for(i = 1; i < argc; i++){
		...
		if(!strcmp(opt, "--m")){
			if(opta)
				plate_params.mu = atof(opta);
			else
				printf("%s: missing argument\n",opt);
		}else if (!strcmp(opt, "--a")){
			if(opta)
				plate_params.adaptation_n = atof(opta);
			else
				printf("%s: missing argument\n",opt);
		}else if (!strcmp(opt, "--s")){
			if(opta)
				plate_params.secondary_n = atof(opta);
			else
				printf("%s: missing argument\n",opt);
		}else if (!strcmp(opt, "--f")){
			if(opta)
				plate_params.feedback_n = atof(opta);
			else
				printf("%s: missing argument\n",opt);
		}
	}
...
int main(int argc, char *argv[]){
	...
	float outputs[getSamplesNumber()][CONTROL_N][NUM_OUTPUTS];
	float inputs[getSamplesNumber()][NUM_ALL_INPUTS];
...
//for(int z = 0; z<1; z++){
	for(;;){
		...
		for(int i=0; i < CONTROL_N; i++){
			...
			copy_from_dsp(dst,dsp_device, dsp_seq - getPackage(), getPackage());
			for(int in = 0; in < getSamplesNumber(); in++){
		...
		double x_full[getSamplesNumber()];

		
		for(int in = 0; in < getSamplesNumber() - plate_params.feedback_n ; in++){
...
//f 0
				...
				for (int ui = 0; ui < plate_params.u; ui++){
					if(in==0){
						for(int fi = 0; fi < plate_params.feedback_n; fi++)
							lf_ring_add(&feedback_ref[ui], outputs[in + fi][j][ui]);
					}else
						lf_ring_add(&feedback_ref[ui], outputs[in + plate_params.feedback_n][j][ui]);
				}
...
//#endif
	...
	for (int num_channel = 0; num_channel < CONTROL_N; num_channel++){
		...
		lf_ring_init(&x, getSamplesNumber());
		lf_ring_set_buffer(&x, NULL, 0);


		for(int k = 0; k < plate_params.secondary_n; k++){
			lf_ring_add(&x, x_full[k]);
		}

		for(int sample = plate_params.secondary_n; sample < getSamplesNumber() - plate_params.feedback_n; sample++){
			...
			do {
				...
				do {
					...
					do {
						...
						r_temp = lf_fir(&x, s[num_channel][ei][ui], plate_params.secondary_n);
